...
const Profil = ({ onLogout, onNavigate }) => {
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      ...
      // Succès — déconnexion et retour splash
      onLogout();
    } catch (err) {
  ...
  // ─── RENDER PRINCIPAL AVEC ANIMATEPRESENCE POUR LES VUES EMPILÉES ───
  return (
    <div className="relative h-full">
      <AnimatePresence mode="wait">
        {legalScreen === 'cgu' && (
          <motion.div 
            ...
            className="h-full absolute inset-0 z-20 bg-night"
          >
            <CGU onBack={() => setLegalScreen(null)} data-stack-back />
          </motion.div>
        )}
        {legalScreen === 'confidentialite' && (
          <motion.div 
            key="confidentialite"
            data-stack-view
            variants={slideVariants}
        ...
        {isEditing && (
          <motion.div 
            ...
            exit="exit"
            transition={{ duration: 0.4, ease: [0.25, 0.1, 0.25, 1] }}
            className="h-full absolute inset-0 z-20 bg-night overflow-y-auto"
          >
      ...
      {/* ─── VUE PRINCIPALE (toujours rendue en dessous) ─── */}
      <div className={`w-full space-y-6 pb-24 animate-in fade-in slide-in-from-bottom-2 duration-700 relative ${isEditing || legalScreen ? 'opacity-0 pointer-events-none' : ''}`}>
        
        {/* Toast d'erreur suppression */}
        <ErrorToast 
          message={deleteError} 
